...
User.create(name: "晶子", email: "<%= ENV["EMAIL4"] %>", password: "<%= ENV["PASSWORD_user"] %>", password_confirmation: "<%= ENV["PASSWORD"] %>", housewife_year: 7, introduction: "今年二人目を出産しました。一人目の子と遊ぶ時間が取れないのが悩みです；")
User.create(name: "りえ", email: "<%= ENV["EMAIL5"] %>", password: "<%= ENV["PASSWORD_user"] %>", password_confirmation: "<%= ENV["PASSWORD"] %>", housewife_year: 3, introduction: "現在妊活中ですが仕事も忙しく思うように行きません。年齢的にも内心焦っています。家事が苦手で仕事も忙しいので精神的に辛いです。同じ境遇だった先輩ママさんからアドバイスいただきたいです")
